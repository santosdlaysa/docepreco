...
const spec = {
  ...
  paths: {
    ...
    '/admin/users': { get: { tags: ['Admin'], summary: 'Listar usuários', security: [{ AdminSecret: [] }], parameters: [{ name: 'search', in: 'query', schema: { type: 'string' } }, { name: 'page', in: 'query', schema: { type: 'integer' } }, { name: 'isPremium', in: 'query', schema: { type: 'boolean' } }, { name: 'signupPlatform', in: 'query', schema: { type: 'string', enum: ['ios', 'android'] } }, { name: 'sortBy', in: 'query', schema: { type: 'string' } }], responses: { 200: { description: 'Lista paginada' } } } },
    ...
    '/admin/users/{id}/signup-platform': { patch: { tags: ['Admin'], summary: 'Atualizar dispositivo de cadastro', security: [{ AdminSecret: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['signupPlatform'], properties: { signupPlatform: { type: 'string', enum: ['ios', 'android'], nullable: true } } } } } }, responses: { 200: { description: 'Dispositivo atualizado' }, 400: { description: 'Valor inválido' }, 404: { description: 'Usuário não encontrado' } } } },
