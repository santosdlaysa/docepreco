  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Receitas Sugeridas (Premium)</h2>
          <p className="text-sm text-gray-500 mt-0.5">Receitas prontas que usuários premium podem usar como base no app</p>
        </div>
                    ...
                    <table className="w-full text-xs min-w-[320px]">
                      ...
                      <tbody className="text-gray-600">
                        {item.ingredients.map((ing, i) => (
                          <tr key={i}>
                            <td className="py-0.5 font-medium text-gray-700">{ing.name}</td>
            ...
            <div className="space-y-3">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={addIngredient} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-primary-50 text-primary-600 hover:bg-primary-100 transition-colors">
                    <Plus size={12} /> Adicionar
                  </button>
                </div>
                {form.ingredients.length === 0 ? (
                  <p className="text-xs text-gray-400 bg-gray-50 rounded-lg p-3 text-center">Nenhum ingrediente adicionado</p>
                ) : (
                  <div className="space-y-2">
                    {form.ingredients.map((ing, i) => (
                      <div key={i} className="flex items-start gap-2 bg-gray-50 rounded-lg p-2">
                        <div className="flex-1 grid grid-cols-2 sm:grid-cols-5 gap-2">
                          <input className="border border-gray-300 rounded px-2 py-1.5 text-xs focus:border-primary-400 outline-none col-span-2 sm:col-span-1"
                            placeholder="Nome" value={ing.name} onChange={e => updateIngredient(i, 'name', e.target.value)} />
                          <input type="number" step="0.1" min="0" className="border border-gray-300 rounded px-2 py-1.5 text-xs focus:border-primary-400 outline-none"
                            placeholder="Qtd usada" value={ing.quantityUsed || ''} onChange={e => updateIngredient(i, 'quantityUsed', parseFloat(e.target.value) || 0)} />
                          <select className="border border-gray-300 rounded px-2 py-1.5 text-xs focus:border-primary-400 outline-none"
                          ...
                          <input type="number" step="0.01" min="0" className="border border-gray-300 rounded px-2 py-1.5 text-xs focus:border-primary-400 outline-none"
