      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
            {user && (
              <button
                onClick={() => { onClose(); onChat(user.id, user.companyName, user.email); }}
...
function UserChatModal({ userId, userName, userEmail, onClose, toast }: {
  userId: string;
  userName: string;
  userEmail: string;
  onClose: () => void;
                ...
                >
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">
                    <span className="flex items-center gap-1.5">
                      <button
                        onClick={e => { e.stopPropagation(); setChatUser({ id: u.id, name: u.companyName, email: u.email }); }}
                        className="text-gray-300 hover:text-primary-500 transition-colors flex-shrink-0"
                        title="Abrir chat com a pessoa"
                        aria-label={`Abrir chat com ${u.companyName}`}
                      >
                        <MessageSquare size={15} />
                      </button>
                      {u.companyName}
