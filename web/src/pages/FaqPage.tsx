  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-xl font-bold text-gray-900">FAQ / Central de Ajuda</h2>
          <p className="text-sm text-gray-500 mt-0.5">Perguntas frequentes exibidas no app</p>
        </div>
              ...
              <div key={item.id} className="hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3 px-5 py-3 cursor-pointer" onClick={() => setExpanded(expanded === item.id ? null : item.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {item.category && <span className="text-xs font-semibold px-2 py-0.5 rounded bg-blue-50 text-blue-600">{item.category}</span>}
                ...
                {expanded === item.id && (
                  <div className="px-5 pb-4 pl-12">
                    <p className="text-sm text-gray-600 whitespace-pre-wrap bg-gray-50 rounded-lg p-3">{item.answer}</p>
                  </div>
                )}
              </div>
            ))}
